...
import { PlayCircle, Moon, Star, Heart, Sparkles, Send, Mail, Music, Menu, X, Youtube, Instagram } from "lucide-react";
...
export default function ClientHomePage({ videos: initialVideos }: { videos: Video[] }) {
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const [videos, setVideos] = useState<Video[]>(initialVideos);

    // Kliencki system pobierania nowości, który działa wszędzie bez względu na to jak silny jest SSG serwera Next.js
    useEffect(() => {
        const fetchVideos = async () => {
            try {
                const channelId = "UCkR8IKcghGGP3_UfmXctbrQ";
                const rssUrl = `https://www.youtube.com/feeds/videos.xml?channel_id=${channelId}`;
                // Proxy AllOrigins aby rozwiązać problem z polityką CORS z YouTube
                const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(rssUrl)}`;

                // Agresywne omijanie cache na kliencie (np. na iOS cache może być silny)
                const res = await fetch(proxyUrl, { cache: 'no-store' });
                if (!res.ok) return; // Silent fail falls back to server initialVideos
                ...
                for (const entry of entries) {
                    ...
                    if (idMatch && titleMatch) {
                        ...
                        const isShorts = /shorts?/i.test(entry);
                        ...
                        if (!isShorts) {
                            ...
                            if (fetchedVideos.length === 3) break;
                        }
                    }
                }

                // Podmień wideo gdy pobrano i upewniłeś się, że są one nowsze/różne od tych statycznych z serwera
                if (fetchedVideos.length > 0 && JSON.stringify(fetchedVideos) !== JSON.stringify(initialVideos)) {
                    setVideos(fetchedVideos);
                }
            } catch {
                // Ignore silent errors for cors/network
            }
        };

        fetchVideos();
        // Odśwież co 5 minut za każdym odpalonym oknem przeglądarki użytkownika!
        const intervalId = setInterval(fetchVideos, 300000);
                            ...
                            {videos.map((video, idx) => (
                                ...
                                >
                                    ...
                                    <div className="relative w-full aspect-video rounded-[2rem] overflow-hidden bg-indigo-50 group">
                                        <div className="absolute inset-0 z-10 touch-pan-y pointer-events-none" style={{ touchAction: 'pan-y' }} onTouchMove={(e) => { if (e.touches.length > 1) e.preventDefault(); }}></div>
                                        <iframe
                                            src={video.url}
                                            title={video.title}
                                            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                                            allowFullScreen
                                            className="absolute inset-0 w-full h-full border-none transition-transform duration-700 relative z-20"
                                        ></iframe>
                                    </div>
            ...
            <footer className="w-full pt-16 pb-8 bg-indigo-950 text-indigo-200 relative z-30">
                <div className="max-w-4xl mx-auto flex flex-col items-center gap-6">
                    ...
                    <div className="flex gap-6 mt-2 items-center justify-center">
                        <a href="https://www.youtube.com/@ziewnutki" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-[#FF0000] hover:scale-110 transition-all duration-300">
                            <span className="sr-only">YouTube</span>
                            <Youtube size={32} />
                        </a>
                        <a href="https://www.tiktok.com/@ziewnutki" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-white hover:drop-shadow-[0_0_8px_rgba(254,44,85,0.8)] hover:scale-110 transition-all duration-300">
                            <span className="sr-only">TikTok</span>
                            <svg width="30" height="30" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 15.68a6.34 6.34 0 006.27 6.36 6.34 6.34 0 006.33-6.33V8.53h.02a8.57 8.57 0 002.97 4.29V9.33a5.18 5.18 0 01-1-2.64z" />
                            </svg>
                        </a>
                        <a href="https://www.instagram.com/ziewnutki" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:text-[#E1306C] hover:scale-110 transition-all duration-300">
                            <span className="sr-only">Instagram</span>
                            <Instagram size={30} />
                        </a>
                        <a href="mailto:kolysanki91@gmail.com" className="text-indigo-400 hover:text-pink-400 hover:scale-110 transition-all duration-300 ml-2 border-l border-indigo-800/50 pl-6">
